...
public class Code2 {
	...
	public void sortAll() {  // 학년별로 학번 오름차순으로 정렬
		for(int i=0; i<sList.size;i++) {
			for(int j=0;j<sList.get(i).size;j++) {
				int max = sList.get(i).size-1;
				if(sList.get(i).get(j).idNum>sList.get(i).get(max).idNum) {
					swap(sList.get(i),j,max);	
				}	
	...
	private void swap(MyArrayList2<ScoreCard> aList, int i, int j) {
		int temp = aList.get(i).idNum;
		aList.get(i).idNum = aList.get(j).idNum;
		aList.get(j).idNum = temp;
	...
	private ScoreCard binarySearch(int gradeIndex, int id, int p, int r) {
		...
		if(p>r) return null;
		int mid = (p+r)/2;
		System.out.println(sList.get(gradeIndex));
		
		if(sList.get(gradeIndex).get(mid).idNum==id) {
			return sList.get(gradeIndex).get(mid);}
		else if(sList.get(gradeIndex).get(mid).idNum<id) return binarySearch(gradeIndex,id,mid+1,r);
		else return binarySearch(gradeIndex, id, p, mid-1);
